...
if token:
    ...
    while i < maxCount:
        try:
            ...
            uri = currTrackId['uri'] 
            theId = currTrackName 
            info = re.sub(r'^\w*:[\w]+:', '', uri)
            playlistIds.append(uri)
            allTrackInfo.append('{\"name\": \"' + currTrackName + '\",\"ids\": \"' + info + '\"}') 
        except: 
    ...
    #pprint.pprint(sp.user('charlesgarza'))
    
    userPlaylist = sp.user_playlist('charlesgarza', playlist_id=myPlaylistId) 
    sp.start_playback(context_uri=userPlaylist['uri'])
